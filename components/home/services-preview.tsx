...
export function ServicesPreview() {
  ...
  useEffect(() => {
    getServices().then((all) => setServices(all.slice(0, 3)))
  }, [])
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
        {services.map((service) => (
          ...
          >
            ...
            <p className={`text-sm leading-relaxed mb-6 ${service.featured ? "opacity-90" : "text-muted-foreground"}`}>
            ...
            <div className="flex items-end justify-between">
              <div>
                ...
                <p className={`text-xs ${service.featured ? "opacity-70" : "text-muted-foreground"}`}>
                  {service.duration}
                </p>
              </div>
            </div>
